...
def get_nested_tasks2(task):
    return [
        object
        for object in [rel.RelatedObjects for rel in task.IsNestedBy]
        if object.is_a("IfcTask")
    ]
    ...
    return [
        {
            ...
            "Name": task.Name,
            "ScheduleStart": format_date_from_iso(task.TaskTime.ScheduleStart)
            if task.TaskTime
            else "",
            "ScheduleFinish": format_date_from_iso(task.TaskTime.ScheduleFinish)
            if task.TaskTime
            else "",
        }
        for task in tasks
        ...
        if "Qto" in pset["Name"]["value"]:
            for quantity in pset["Quantities"]:
                ...
                dict[pset["expressID"]]["Data"].append(
                    {"Name": quantity_name, "Value": quantity_value}
                )
