...
def load_data():
    if "ifc_file" in session:
        session.Graphs = {
            ...
            "high_frquency_graph": graph_maker.get_high_frequency_entities_graph(
                session.ifc_file
            ),
        }
        load_cost_schedules()
...
def draw_schedules():
    ...
    with col1:
        ...
        schedules = [
            f"{work_schedule.Name} / {work_schedule.id()}"
            for work_schedule in session.SequenceData["schedules"] or []
        ]
        st.selectbox("Schedules", schedules, key="schedule_selector")
        schedule_id = (
            int(session.schedule_selector.split("/", 1)[1])
            if session.schedule_selector
            else None
        )
    ...
    with col2:
        ...
        st.selectbox(
            "Cost Schedules",
            [
                f"{cost_schedule.Name} ({cost_schedule.id( )})"
                for cost_schedule in session.CostData["schedules"] or []
            ],
        )
...
def draw_side_bar():
    ...
    st.sidebar.button(
        "➕ Add Schedule", key="add_schedule_button", on_click=add_cost_schedule
    )
    ...
    st.sidebar.button(
        "➕ Add Schedule", key="add_work_schedule_button", on_click=add_work_schedule
    )
